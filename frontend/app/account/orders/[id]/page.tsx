...
function getStatusColor(status: string) {
  ...
  if (status === "CANCELLED" || status === "FAILED" || status === "PAYMENT_FAILED") return { bg: "#fef2f2", text: "#dc2626", border: "#ef4444" };
...
export default function CustomerOrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function handleDownloadInvoice() {
    ...
    try {
      ...
      if (res?.invoice_url) {
        ...
        toast.error("Official Shiprocket tax invoice is being generated. Please check back shortly.");
      }
    } catch (err: any) {
      console.warn("Failed to fetch official Shiprocket invoice", err);
      toast.error(err?.message || "Official Shiprocket tax invoice is being generated by courier. Please try again shortly.");
    } finally {
  ...
  const isFailed = order.status === "FAILED" || order.status === "PAYMENT_FAILED" || order.paymentStatus === "FAILED";
        ...
        >
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 14, marginBottom: 16 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <div>
                <h3 style={{ fontSize: "1.1rem", fontWeight: 900, textTransform: "uppercase", letterSpacing: "-0.02em", margin: 0, color: "#111" }}>
                  Payment Unsuccessful
                </h3>
                <p style={{ margin: "2px 0 0", fontSize: "0.8rem", color: "#666" }}>
                  {order.cancellationReason || "The payment transaction was declined by your bank or the session expired."}
                </p>
              </div>
            </div>
          ...
          <div style={{
            ...
            <strong style={{ color: "#c53030" }}>Notice on Deductions:</strong> No funds were captured by VAHN for this order. If your bank or UPI app debited this amount, the issuing bank will automatically release the hold and credit it back to your account within 5–7 business days.
          </div>
